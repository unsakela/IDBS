"""
Mendeley Driving Dataset Loader for PyTorch

This script provides a PyTorch Dataset and DataLoader for the processed Mendeley driving data.
"""
...
class MendeleyDrivingDataset(Dataset):
    """
    PyTorch Dataset for Mendeley driving data.
    ...
    def __init__(self, data_path: str, split: str = 'train', 
                 ...
                 random_seed: int = 42):
        """
        Initialize Mendeley driving dataset.
        
        Args:
            data_path: Path to processed Mendeley driving data file
        ...
        print(f"Loaded Mendeley Driving {split} dataset:")
...
def create_mendeley_driving_dataloaders(data_path: str, 
                                      batch_size: int = 32,
                                      num_workers: int = 4,
                                      split_ratio: Tuple[float, float, float] = (0.8, 0.1, 0.1),
                                      random_seed: int = 42) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """
    Create PyTorch DataLoaders for Mendeley driving dataset.
    
    Args:
        data_path: Path to processed Mendeley driving data
    ...
    train_dataset = MendeleyDrivingDataset(data_path, 'train', split_ratio, random_seed)
    val_dataset = MendeleyDrivingDataset(data_path, 'val', split_ratio, random_seed)
    test_dataset = MendeleyDrivingDataset(data_path, 'test', split_ratio, random_seed)
    ...
    print(f"Mendeley Driving DataLoaders created:")
...
def test_mendeley_driving_loader():
    """Test Mendeley driving dataset loader."""
    print("Testing Mendeley Driving Dataset Loader...")
    
    try:
        # Create data loaders
        train_loader, val_loader, test_loader = create_mendeley_driving_dataloaders(
            "data/processed/mendeley_driving/processed.pkl",
            batch_size=32,
            num_workers=2
        )
        ...
        if len(train_loader) > 0:
            ...
            print(f"Loaded Mendeley Driving train dataset:")
        ...
        print(f"Error testing Mendeley Driving loader: {e}")
        print("This is expected if the dataset hasn't been preprocessed yet.")
    
    print("✓ Mendeley Driving Dataset Loader test completed!")


if __name__ == "__main__":
    test_mendeley_driving_loader()
