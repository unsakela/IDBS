"""
Mendeley Risky Dataset Loader for PyTorch

This script provides a PyTorch Dataset and DataLoader for the processed Mendeley risky data.
"""
...
class MendeleyRiskyDataset(Dataset):
    """
    PyTorch Dataset for Mendeley risky data.
    
    Loads preprocessed risky driving data with sliding window features.
    """
    ...
    def __init__(self, data_path: str, split: str = 'train', 
                 ...
                 random_seed: int = 42):
        """
        Initialize Mendeley risky dataset.
        
        Args:
            data_path: Path to processed Mendeley risky data file
        ...
        print(f"Loaded Mendeley Risky {split} dataset:")
...
def create_mendeley_risky_dataloaders(data_path: str, 
                                     batch_size: int = 32,
                                     num_workers: int = 4,
                                     split_ratio: Tuple[float, float, float] = (0.8, 0.1, 0.1),
                                     random_seed: int = 42) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """
    Create PyTorch DataLoaders for Mendeley risky dataset.
    
    Args:
        data_path: Path to processed Mendeley risky data
    ...
    train_dataset = MendeleyRiskyDataset(data_path, 'train', split_ratio, random_seed)
    val_dataset = MendeleyRiskyDataset(data_path, 'val', split_ratio, random_seed)
    test_dataset = MendeleyRiskyDataset(data_path, 'test', split_ratio, random_seed)
    ...
    print(f"Mendeley Risky DataLoaders created:")
...
def test_mendeley_risky_loader():
    """Test Mendeley risky dataset loader."""
    print("Testing Mendeley Risky Dataset Loader...")
    
    try:
        # Create data loaders
        train_loader, val_loader, test_loader = create_mendeley_risky_dataloaders(
            "data/processed/mendeley_risky/processed.pkl",
            batch_size=32,
            num_workers=2
        )
        ...
        if len(train_loader) > 0:
            ...
            print(f"Loaded Mendeley Risky train dataset:")
        ...
        print(f"Error testing Mendeley Risky loader: {e}")
        print("This is expected if the dataset hasn't been preprocessed yet.")
    
    print("✓ Mendeley Risky Dataset Loader test completed!")


if __name__ == "__main__":
    test_mendeley_risky_loader()
